...
public:
    ...
    std::vector<Value> biuld_array_from_root(std::vector<Value> result)
    {
        result = biuld_array(root, result);
        return result;
    }

private:
    // Обходим дерево в порядке in-order
    std::vector<Value> biuld_array(Node *node, std::vector<Value> &result)
    {
        if (node->left)
        {
            result = biuld_array(node->left, result);
        }
        result.push_back(node->value);
        if (node->right)
        {
            result = biuld_array(node->right, result);
        }
        return result;
    }
